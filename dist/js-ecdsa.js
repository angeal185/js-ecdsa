...
const ecdsa = {
  ...
  sign: function(key, data, digest, cb){
    ...
    .then(function(i){
      window.crypto.subtle.sign({name: "ECDSA", hash: {name: "SHA-" + digest}}, i, ecdsa.str2u8(data))
      .then(function(signature){
        //console.log(signature)
        let sig = ecdsa.a2h(new Uint8Array(signature));
        cb(false,sig);
      })
      .catch(function(err){
          cb(err);
  ...
  verify: function(key, sig, data, digest, cb){
    window.crypto.subtle.importKey("jwk",
      key,{
        name: "ECDSA",
        namedCurve: key.crv
      ...
      window.crypto.subtle.verify({name: "ECDSA", hash: {name: "SHA-"+digest}},
          i,
          new Uint8Array(ecdsa.h2a(sig)),
          ecdsa.str2u8(data)
      )
